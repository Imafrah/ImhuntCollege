import { CollegeType, type College, type CourseFee, type PlacementStat, type Review } from "@prisma/client";
...
type ComparisonCollege = College & {
...
function parseCompareIds(rawIds: string): { ids: number[]; requestedCount: number } | null {
...
function logResponseTime(path: string, responseTimeMs: number): void {
...
function aggregateRatings(reviews: Review[]): RatingAverages {
  return {
...
function lowestWinner(colleges: ComparisonCollege[], getValue: (college: ComparisonCollege) => number | null): ComparisonWinner {
  const ranked = colleges
    .map((college) => ({ college, value: getValue(college) }))
    .filter((entry): entry is { college: ComparisonCollege; value: number } => entry.value !== null)
...
function highestWinner(colleges: ComparisonCollege[], getValue: (college: ComparisonCollege) => number | null): ComparisonWinner {
  const ranked = colleges
    .map((college) => ({ college, value: getValue(college) }))
    .filter((entry): entry is { college: ComparisonCollege; value: number } => entry.value !== null)
...
router.get("/", async (req, res, next) => {
  ...
  try {
    ...
    const summaries: CollegeSummary[] = colleges.map((college) => ({
      id: college.id,
      name: college.name,
      city: college.city,
      state: college.state,
...
router.get("/compare", async (req, res, next) => {
  try {
    ...
    const comparisonColleges: ComparisonCollege[] = colleges.map((college) => ({
      ...college,
